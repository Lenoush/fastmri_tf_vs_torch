...
def evaluate_xpdnet(
    ...
    if brain:
        ...
        n_volumes = len([name for name in os.listdir(val_path)])
        if contrast is not None:
            n_volumes //= 2
            n_volumes += 1
    ...
    if distributed and not manual_saving:
        ...
        model(inputs)
        model.load_weights(f'/volatile/Lena/Codes/Modèles/fastmri-reproducible-benchmark/result/checkpoints/xpdnet_singlecoil__af4_MWCNNbig_1709892662-02.hdf5')
